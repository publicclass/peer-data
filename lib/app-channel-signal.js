...
function embed(prefix, bridge, fn){
  ...
  if( bridge ){
    ...
    iframe.height = 0;
    iframe.onerror = fn;
    iframe.onload = function(){ fn(null, iframe); };
...
function bridge(prefix, token, element){
  ...
  element.contentWindow.postMessage({token: token}, prefix);
  ...
  function onmessage(e){
    // validate the origin
    if( e.origin !== prefix ){
      console.log('(ignored) message from bridge', e);
      return;
    }
    console.log('message from bridge', e);
