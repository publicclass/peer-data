    ...
    if( json.type == 'connected' ){
      ...
      if( peer ){
        if( json.type == 'request-for-offer' ){
          console.log('received a request-for-offer. %s <> %s', json.challenge, peer.challenge, json.challenge < peer.challenge);
          if( json.challenge < peer.challenge ){
            console.log('won request challenge. will create and send offer.', peer.peerConnection.signalingState);
            peer.peerConnection.createOffer(function(desc){
              console.log('created offer.', peer.peerConnection.signalingState);
              peer.peerConnection.setLocalDescription(desc, function(){
                console.log('set offer. now sending it.', peer.peerConnection.signalingState);
          ...
          console.log('received offer', peer.peerConnection.signalingState);
          var desc = new RTCSessionDescription(json);
          peer.peerConnection.setRemoteDescription(desc, function(){
            console.log('successfully set remote offer', peer.peerConnection.signalingState);
            peer.peerConnection.createAnswer(function(desc){
              console.log('successfully created answer', peer.peerConnection.signalingState);
              peer.peerConnection.setLocalDescription(desc, function(){
                console.log('set answer. now sending it.', peer.peerConnection.signalingState);
                this.signal.send(desc, from);
              }, function(err){
          ...
          console.log('received answer', peer.peerConnection.signalingState);
          var desc = new RTCSessionDescription(json);
          peer.peerConnection.setRemoteDescription(desc, function(){
            console.log('successfully set remote answer!', peer.peerConnection.signalingState);
          }, function(err){
          ...
          if( json.candidate ){
            console.log('received icecandidate', peer.peerConnection.signalingState);
            var candidate = new RTCIceCandidate(json.candidate);
            peer.peerConnection.addIceCandidate(candidate, function(){
