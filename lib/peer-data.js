...
PeerData.prototype.open = function() {
  ...
  this.signal.on('turn', function(config) {
    this.options.servers.iceServers.push(config);
  }.bind(this));

  this.signal.on('open', function() {
    ...
    if (json.type == 'connected') {
      ...
      if (peer) {
        if (json.type == 'request-for-offer') {
          ...
          var odesc = new RTCSessionDescription(json);
          ...
          var adesc = new RTCSessionDescription(json);
  ...
  if (this.signal.state != 'open') {
    ...
    console.warn('signal was already open. we need to fetch "from"');
    this.from = this.signal.peer;
  }
...
PeerData.prototype.addPeer = function(id) {
  ...
  connection.oniceconnectionstatechange = function(e) {
    debug('iceconnectionstatechange -> %s', connection.iceConnectionState);

    // TODO if iceConnectionState is converted into 'completed' or 'connected'
    //      we create a reliable data channel with which we should send any
    //      future negotiations over. We could even replace the current signal
    //      completely and silently close the app channel.
    //
    //      Maybe something like:
    //
    //        var newSignal = new DataChannelSignal(signal.room);
    //        newSignal.on('open', function(){
    //          // rewire all signal events and replace the internal instance
    //          this.signal = this.signal.upgrade(newSignal);
    //        });
